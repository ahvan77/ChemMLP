...
from PySide6.QtWidgets import QApplication, QMainWindow, QMenuBar, QMenu, QStatusBar,QToolBar, QLabel, QMessageBox, QFileDialog
#from PyQt6.QtWidgets import QApplication, QMainWindow.setStatusBar
...
#        self.myStatusBar.addPermanentWidget(self.molcounter, 0)
    ...
    def CreateMenus(self):
        ...
        self.toolMenu = QMenu("&Tools")
        self.atomtypeMenu = QMenu("&AtomTypes")
        self.bondtypeMenu = QMenu("&BondTypes")
        self.helpMenu = QMenu("&Help")
        self.menubar.addMenu(self.fileMenu)
        self.menubar.addMenu(self.toolMenu)
        self.menubar.addMenu(self.atomtypeMenu)
        self.menubar.addMenu(self.bondtypeMenu)
    ...
    #    self.toolMenu.addAction(self.rsAction)
     ...
     #   self.toolMenu.addAction(self.decreaseChargeAction)
        ...
        self.toolMenu.addAction(self.descriptorsAction)
     ...
     #   self.mainToolBar.addAction(self.increaseChargeAction)
      #  self.mainToolBar.addAction(self.decreaseChargeAction)
       ...
       # self.mainToolBar = QtWidgets.QToolBar(self)
        ...
        self.mainToolBar.addAction(self.singleBondAction)
        self.mainToolBar.addAction(self.doubleBondAction)
        self.mainToolBar.addAction(self.tripleBondAction)
        self.mainToolBar.addSeparator()
   #     for action in self.atomActions:
    #        self.sideToolBar.addAction(action)
        self.mainToolBar.addAction(self.openPtableAction)
        self.mainToolBar.addSeparator()
        self.mainToolBar.addAction(self.descriptorsAction)
...
#            file.write(self.textEdit.toPlainText())
    ...
    def descriptorInsert(self):
        try:
                with open(self.pixmappath + 'Molecule.mol', 'w') as f:
                    f.write(Chem.MolToMolBlock(self.editor.mol))
                subprocess.run(["python", "descriptor.py"])    
             #   descriptor.PropertiesDialog('Molecoule.mol')
    ...
    def exitFile(self):
        ...
        if response == "Y":
            self.ptable.close()
            sys.exit(0) #TODO, how to exit qapplication from within class instance?
        else:
    ...
    def CreateActions(self):
        self.openAction = QAction( QIcon(self.pixmappath0+ 'open_file.jpg'), 'O&pen',
                                  self, shortcut=QKeySequence.Open,
                                  statusTip="Open an existing file",
                                  triggered=self.openFile)

        self.saveAction = QAction( QIcon(self.pixmappath0+ '/save.jpg'), 'S&ave',
                                  self, 
        ...
        self.saveAsAction = QAction( QIcon(self.pixmappath0+ 'Save-as-icon.png'), 'Save As',
                                  self, 
        ...
        self.aboutAction = QAction( QIcon(self.pixmappath0 + 'about.png'), 'A&bout',
        ...
        self.openPtableAction = QAction( QIcon(self.pixmappath0 + 'periodic.png'), 'O&pen Periodic Table',
                                  self, shortcut=QKeySequence.Open,
                                  statusTip="Open the periodic table for atom type selection",
                                  triggered=self.openPtable)

        #Edit actions
        self.actionActionGroup = QActionGroup(self, exclusive=True)
        self.selectAction = QAction( QIcon(self.pixmappath0 + 'select.png'), 'Se&lect',
        ...
        self.addAction = QAction( QIcon(self.pixmappath0 + 'add.png'), '&Add Atom',
                                   self, shortcut="Ctrl+A",
                                   statusTip="Add Atoms",
                                   triggered=self.setAction, objectName="Add Atom",
                                   checkable=True)
        self.actionActionGroup.addAction(self.addAction)

        self.addBondAction = QAction( QIcon(self.pixmappath0 + 'add_bond.png'), 'Add &Bond',
        ...
        self.replaceAction = QAction( QIcon(self.pixmappath0 + 'replace.png'), '&Replace',
  ...
  #      self.rsAction = QAction( QIcon(self.pixmappath0 + 'Change_R_S.png'), 'To&ggle R/S',
  ...
  #      self.ezAction = QAction( QIcon(self.pixmappath0 + 'Change_E_Z.png'), 'Toggle &E/Z',
   #                                self, shortcut="Ctrl+E",
    #                               statusTip="Toggle Bond Stereo Chemistry",
     #                              triggered=self.setAction, objectName="EZtoggle",
      ...
      #  self.actionActionGroup.addAction(self.ezAction)
        ...
        self.removeAction = QAction( QIcon(self.pixmappath0 + 'delete.png'), 'D&elete',
    ...
    #    self.increaseChargeAction = QAction( QIcon(self.pixmappath0 + 'icons8-Increase Font.png'), 'I&ncrease Charge',
     #                              self, shortcut="Ctrl++",
      ...
      #  self.decreaseChargeAction = QAction( QIcon(self.pixmappath0 + 'icons8-Decrease Font.png'), 'D&ecrease Charge',
       #                            self, shortcut="Ctrl+-",
     ...
     #   self.actionActionGroup.addAction(self.decreaseChargeAction)
        ...
        #BondTypeActions
        self.bondtypeActionGroup = QActionGroup(self, exclusive=True)

        self.singleBondAction = QAction( QIcon(self.pixmappath0 + 'single.png'), 'S&ingle Bond',
                                   self, shortcut="Ctrl+1",
                                   statusTip="Set bondtype to SINGLE",
                                   triggered=self.setBondType, objectName="SINGLE",
                                   checkable=True)
        
        self.descriptorsAction = QAction( QIcon(self.pixmappath0 + 'descriptor.png'), 'D&escriptors Value',
                                   self, shortcut="Ctrl+1",
                                   statusTip="Set descriptor values",
                                   triggered=self.descriptorInsert, objectName="Descriptors",
                                   checkable=True)
        
        self.bondtypeActionGroup.addAction(self.singleBondAction)

        self.doubleBondAction = QAction( QIcon(self.pixmappath0 + 'double.png'), 'Double Bond',
        ...
        self.tripleBondAction = QAction( QIcon(self.pixmappath0 + 'triple.jpg'), 'Triple Bond',
        ...
        #Misc Actions
        self.undoAction = QAction( QIcon(self.pixmappath0 + 'Undo.png'), 'U&ndo',
                           self, shortcut="Ctrl+Z",
                           statusTip="Undo/Redo changes to molecule Ctrl+Z",
                           triggered=self.editor.undo, objectName="undo")

        self.clearCanvasAction = QAction( QIcon(self.pixmappath0 + 'clear.jpg'), 'C&lear Canvas',
                                   self, shortcut="Ctrl+X",
                                   statusTip="Clear Canvas (no warning)",
                                   triggered=self.clearCanvas, objectName="Clear Canvas")

        self.cleanCoordinatesAction = QAction( QIcon(self.pixmappath0 + 'recalculate.jpg'), 'Recalculate coordinates &F',
