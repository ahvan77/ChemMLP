...
def data_xy(input_file):
    ...
    if 'SMILES' in target:
        ...
        num_tokens = len(tokenizer.word_index) + 1  # Add one for padding token
        # Encode SMILES strings as sequences of tokens
        ...
        # One-hot encode sequences (target labels)
        target1 = tf.keras.utils.to_categorical(padded_sequences, num_classes=num_tokens)
        y= np.array(target1)
      #  y = df[target]
    else:
        y = df[target].values     
