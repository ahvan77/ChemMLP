...
class MolEditWidget(MolWidget):
    def __init__(self, mol = None, parent=None):
        ...
        self.points = [Point3D(0,0,0),Point3D(1,1,0), Point3D(0,0,0)]
        ...
        self.mol = mol
   #     self.action_lookup = {
    #        "Add": self.add_atom_to,
     #       "Remove": self.remove_atom,
      #      "Select": self.select_atom_add,
       #     "Replace": self.replace_atom,
        #    "Add Bond": self.add_bond,
         #   "Increase Charge": self.increase_charge,
          #  "Decrease Charge": self.decrease_charge,
          #  "RStoggle": self.toogleRS,
            # ... add more actions as needed
          #  }

      #  self.context_lookup = {
       #     "Atom": self.atom_click,
       #     "Bond": self.bond_click,
       #     "Canvas": self.canvas_click,
    ...
    def SVG_to_coord(self, x_svg, y_svg, z_svg):
        if self.drawer != None:
            ...
            az = scale1.z - scale0.z
            bz = scale0.z
            return Point3D((x_svg-bx)/ax, (y_svg-by)/ay, (z_svg-bz)/az)
        else:
    ...
    def get_nearest_atom(self,x_svg, y_svg, z_svg):
        if self.mol != None and self.mol.GetNumAtoms() > 0:
            atomsvgcoords = np.array([x_svg,y_svg, z_svg])
    ...
    def get_nearest_bond(self, x_svg, y_svg, z_svg):
    ...
    def get_molobject(self, event):
        ...
        z_svg = float(z)/size.height()*viewbox.height()+viewbox.top()
        self.logger.debug("SVG_coords:\t%s\t%s"%(x_svg, y_svg, z_svg))
        #Identify Nearest atomindex
        atom_idx, atom_dist = self.get_nearest_atom(x_svg, y_svg, z_svg)
        bond_idx, bond_dist = self.get_nearest_bond(x_svg, y_svg, z_svg)
        ...
        if min([atom_dist, bond_dist]) < 14.0:
            ...
            return self.SVG_to_coord(x_svg, y_svg, z_svg)
...
#                        Chem.rdchem.ChiralType.CHI_OTHER, this one doesn't show a wiggly bond
        ...
        ##rdDepictor.Compute2DCoords(self._mol)
        AllChem.EmbedMolecule(self._mol, randomSeed=42)        
